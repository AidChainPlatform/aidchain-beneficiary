...
class CampaignAPI extends BaseService {
  ...
  List<dynamic> _asList(dynamic body) {
    ...
    if (body is Map<String, dynamic>) {
      final v = body['data'] ?? body['campaigns'] ?? body['items'] ?? body['results'];
  ...
  Future<List<dynamic>> getAllPublicCampaigns() async {
    final url = '${BaseService.rootApi}/campaigns';
    ...
    try {
      ...
      if (kDebugMode) {
        print({"SENDING REQUEST TO API.... TO GET public campaigns"});
        print({"RAW_BODY_TYPE": response.data.runtimeType.toString()});
      }

      return _asList(response.data); // ← ALWAYS return a List
    } on DioError catch (e) {
      if (e.response != null) {
        if (kDebugMode) {
        ...
        // propagate as empty list so UI doesn't crash
        return <dynamic>[];
      } else {
        if (kDebugMode) {
