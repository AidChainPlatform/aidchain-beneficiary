...
// New API implementation
  ...
  // ✅ NEW: Decode QR code from uploaded image
  decodeQrFromImage(File image) async {
    String _decodeQrURL = BaseService.rootApi + '/utils/decode-qr';
    ...
    try {
      ...
      if (kDebugMode) print({"QR DECODE RESPONSE", response.data});
      return response.data['data'];
    } on DioError catch (e) {
      if (e.response != null) {
        print(e.response!.data);
        return {"status": "failed", "message": e.response!.data['message'] ?? e.message};
      } else {
        print(e.requestOptions);
        print(e.message);
        return {"status": "failed", "message": e.message};
  ...
  // ✅ NEW: Scan & Pay from QR payload
  scanPay(int campaignId, double amount, Map<String, dynamic> qrPayload) async {
    String _scanPayURL = BaseService.rootApi + '/vendors/orders/scan-pay';
    ...
    try {
      ...
      if (kDebugMode) print({"SCAN PAY RESPONSE", response.data});
      return response.data['data'];
    } on DioError catch (e) {
      if (e.response != null) {
        print(e.response!.data);
  ...
  // ✅ NEW: Decode QR and process payment automatically (one-step)
  decodeQrAndPay(File image) async {
    String _decodeQrAndPayURL = BaseService.rootApi + '/utils/decode-qr-and-pay';

    try {
      print('Uploading QR image for automatic payment processing...');
      ...
      if (kDebugMode) print({"QR DECODE AND PAY RESPONSE", response.data});
      return response.data['data'];
    } on DioError catch (e) {
      if (e.response != null) {
        print(e.response!.data);
