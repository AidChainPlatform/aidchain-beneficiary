...
Future<void> setUserData() async {
  final detailsResponse = await UserAPI().getUserDetailsAndWallet();

  // --- FORCE location to a String (JSON text), no matter what backend sends.
  try {
    final rawLoc = detailsResponse['location'];
    if (rawLoc == null) {
      detailsResponse['location'] = null;
    } else if (rawLoc is String) {
      // if it is already a JSON string but sometimes plain text — try normalize once
      try {
        final asMap = jsonDecode(rawLoc);
        if (asMap is Map) {
          detailsResponse['location'] = jsonEncode(asMap);
        } else {
          // leave as-is; model will handle it
          detailsResponse['location'] = rawLoc;
        }
      } catch (_) {
      ...
      detailsResponse['location'] = jsonEncode(rawLoc);
    } else {
      detailsResponse['location'] = null;
    }
  } catch (e) {
    // If anything goes wrong, don't let a Map slip through
    detailsResponse['location'] = null;
  }

  // --- (Optional) ensure Wallets is a List<Map<String,dynamic>>
  final wallets = detailsResponse['Wallets'];
  if (wallets is List) {
    detailsResponse['Wallets'] =
        wallets.map((w) => Map<String, dynamic>.from(w as Map)).toList();
  }

  // --- DEBUG: show types so we can verify normalization happened
  if (kDebugMode) {
    print({
      'Setting user data...',
      detailsResponse['location']?.runtimeType ?? 'null',
      detailsResponse['Wallets'],
    });
  }

  // --- Parse model with crash guard so we see exactly what failed if anything else is off
  try {
    _data = UserDetails.fromJson(detailsResponse);
  } catch (e, st) {
    // Deep diagnostics to find the field causing trouble
    print('❌ Failed to parse UserDetails: $e');
    print(st);
    detailsResponse.forEach((k, v) {
  ...
  Future<String?> setUserPIN(String? pin) async {
    final dynamic resp = await UserAPI().setUsersTransactionPIn(pin);
    if (kDebugMode) print('Setting user pin... $resp');

    // On success, refresh user data so UI reflects latest flags/state
    final msg = (resp is String)
        ? resp
        : (resp is Map && resp.containsKey('message'))
  ...
  Future<String?> updatePIN(String? oldPin, String? newPin) async {
    final dynamic resp = await UserAPI().updateUsersTransactionPIn(oldPin, newPin);
    if (kDebugMode) print('Updating user pin... $resp');
...
Future<void> retrieveTransactions(String period) async {
  ...
  debugPrint("📝 Raw transaction response: $response");

  if (response['status'] != "failed") {
    _history = TransactionsHistory.fromJson(response['data']);
    debugPrint("✅ Parsed history: ${_history?.transactions?.count}");
  } else {
    debugPrint("❌ Transaction API failed: ${response['message']}");
  }
}
